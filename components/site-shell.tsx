...
export function SiteShell({children}: {children: ReactNode}) {
  ...
  return <><a href="#main" className="skip" onClick={e => {e.preventDefault(); document.getElementById('main')?.focus();}}>Lewati ke konten</a><header className="header"><div className="nav-wrap"><Link className="brand" href="/" aria-label="OTENTIK beranda"><Image src="/assets/logo.svg" width={156} height={38} alt="OTENTIK" loading="eager"/></Link><nav aria-label="Navigasi utama">{[['/','Beranda'],['/katalog','Jelajahi karya'],['/lisensi','Lisensi saya']].map(([href,label]) => {const active = href === '/' ? path === '/' : path.startsWith(href); return <Link key={href} href={href} className={active?'active':''} aria-current={active?'page':undefined}>{label}{href === '/lisensi' && <> <span className="count" id="license-count">{purchases.length}</span></>}</Link>;})}</nav><Link className="button small studio-nav" href="/studio" aria-current={path==='/studio'?'page':undefined}>Studio kreator <span aria-hidden="true">↗</span></Link></div></header><main id="main" tabIndex={-1}>{children}</main><footer><div className="container footer-inner"><div><Image src="/assets/logo.svg" width={126} height={31} alt="OTENTIK"/><p>Karya punya cerita. Kreator punya hak.</p></div></div></footer><Intro/><Motion/></>;
}
